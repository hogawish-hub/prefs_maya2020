...
import cStringIO, re, polySelectionUtils, decimal, types
...
class SkinningTools(object):
    ...
    def freezeSkinnedJointsFull(self, joints):
        info = OrderedDict()
        for jnt in joints:
            children = cmds.listRelatives(jnt, ad=1)
            info[jnt] = children
            if children is not None:
                cmds.parent(children, w=1)
            
            loc = cmds.spaceLocator()[0]
            cmds.matchTransform(loc, jnt, pos=1, rot=1)
            ...
            try: 
                ...
                if parent:
                    ...
                    cmds.matchTransform(jnt, loc, pos=1, rot=1)
